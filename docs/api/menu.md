...
※ 인기 메뉴는 Redis ZSet 기반으로 관리되며, 결제 성공 시 해당 날짜 기준으로 즉시 반영된다.

※ 조회 시 최근 7일간의 일별 집계 데이터를 합산하여 상위 3개 메뉴를 반환한다.

※ 주문 취소(`CANCELED_BY_USER`, `CANCELED_BY_SYSTEM`)된 데이터는 인기 메뉴 집계에서 제외되며, 주문이 반영된 날짜 기준으로 score를 차감한다.
